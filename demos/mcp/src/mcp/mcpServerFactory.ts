...
export const mcpServerFactory = (d: DependencyContainer) => {
    ...
    tools.map((tool) => {
        ...
        const rules = injector
            .getAll(MCP_ARG_TOKEN)
            .filter((it) => it.target === tool.target);
        ...
        mcpServer.registerTool(
            ...
            async (args, extra) => {
                try {
                    ...
                    throw new Error(
                        `method ${tool.property.toString()} not found`
                    );
                } catch (e) {
    ...
    resources.map((res) => {
        ...
        if (typeof options.uriOrTemplate === "string") {
            ...
            mcpServer.registerResource(
                options.name,
                options.uriOrTemplate,
                options.config,
                cb
            );
        } else {
            const cb: ReadResourceTemplateCallback = async (
                uri,
                variables,
                extra
            ) => {
    ...
    prompts.map((prompt) => {
        ...
        const rules = injector
            .getAll(MCP_ARG_TOKEN)
            .filter((it) => it.target === prompt.target);
        let argsRule: ZodRawShape = {};
        const params = rules
            .sort((a, b) => a.parameterIndex - b.parameterIndex)
            .map((rule) => {
