...
export function getTableName<T>(entityClass: new () => T): string {
    try {
        // 从容器中获取实体元数据
        const entityMetadata = container.resolveAll(ENTITY_TOKEN)
            .find((metadata: ClassMetadata<EntityOptions>) => metadata.target === entityClass);

        if (entityMetadata && entityMetadata.options.tableName) {
            return entityMetadata.options.tableName;
        }
...
export function getEntityColumns<T>(entityClass: new () => T): Array<{
    ...
    try {
        ...
        // 获取所有列装饰器的元数据
        const columnTokens = [COLUMN_TOKEN, PRIMARY_COLUMN_TOKEN, PRIMARY_GENERATED_COLUMN_TOKEN];

        for (const token of columnTokens) {
            const columnMetadataList = container.resolveAll(token)
                .filter((metadata: PropertyMetadata<ColumnOptions>) => metadata.target === entityClass);

            for (const metadata of columnMetadataList) {
                columns.push({
                    propertyName: metadata.property as string,
                    columnName: metadata.options.name || metadata.property as string,
                    type: metadata.options.type || ColumnType.VARCHAR,
...
export function getEntityRelations<T>(entityClass: new () => T): Array<{
    ...
    try {
        ...
        const relationTokens = [
            { token: ONE_TO_ONE_TOKEN, type: RelationType.ONE_TO_ONE },
            { token: ONE_TO_MANY_TOKEN, type: RelationType.ONE_TO_MANY },
            { token: MANY_TO_ONE_TOKEN, type: RelationType.MANY_TO_ONE },
            { token: MANY_TO_MANY_TOKEN, type: RelationType.MANY_TO_MANY }
        ];

        for (const { token, type } of relationTokens) {
            const relationMetadataList = container.resolveAll(token)
                .filter((metadata: PropertyMetadata<BaseRelationOptions>) => metadata.target === entityClass);

            for (const metadata of relationMetadataList) {
                // 获取目标实体类型
                let targetEntity: Function;
                if (typeof metadata.options.type === 'function') {
                    targetEntity = metadata.options.type();
                } else if (metadata.options.type) {
                    targetEntity = metadata.options.type;
                } else {
                    // 如果没有指定类型，跳过这个关系
                    continue;
 ...
 * 获取实体的索引信息
 */
export function getEntityIndexes<T>(entityClass: new () => T): Array<{
    name?: string;
    columns: string[];
    unique?: boolean;
    ...
    try {
        ...
        // 获取索引装饰器的元数据
        const indexMetadataList = container.resolveAll(INDEX_TOKEN)
            .filter((metadata: ClassMetadata<IndexOptions>) => metadata.target === entityClass);

        for (const metadata of indexMetadataList) {
            indexes.push({
                name: metadata.options.name,
                columns: metadata.options.columns || [],
